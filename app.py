from flask import Flask, redirect, url_for, session, Blueprint, jsonify, render_template, request
...
def getData():
    user = Allusers.query.filter_by(email="mamun.cseian@gmail.com").first()
    if not user:
        admin = Allusers(email="mamun.cseian@gmail.com", is_admin=True, nickname="Mamun")
...
def addUser():
    try:
        ...
        if db_user:
            ...
            new_user = Allusers(email=user["email"], is_admin=user["is_admin"], nickname=None)
